...
head of patients (\refapdx{apdx:supplemental}{tab:S1}) undergoing elective hip
...
The plasmacytoma cell line INA-6 [\textit{RRID:CVCL\_5209}; DSMZ, Braunschweig,
...
Germany). INA-6 were passaged three times per week by diluting them to
...
\subsubsection*{Co-Culturing of Primary hMSCs and INA-6 and MSC-Conditioning of Medium}
For each co-culture, hMSCs were seeded out \SI{24}{\hour} prior to INA-6
addition to generate MSC-conditioned medium (CM). CM from different donors was
collected separately and used immediately when adding INA-6. To ensure that CM
was free of hMSCs, it was strained (\SI{40}{\micro\meter}) and centrifuged for
\SI{15}{\minute} at \SI{250}{g}. INA-6 cells were washed with PBS
...
For saturating hMSC with INA-6, hMSCs were stained with \textit{CellTracker
    Green}, plated out on 384-well plates (Greiner Bio-One GmbH, Frickenhausen,
Germany) at \SI{5e3}{hMSC/cm^2} and cultured for \SI{24}{\hour}. INA-6 cells
...
\SI{48}{\hour}. The complete co-culture was scanned and the number of INA-6
cells adhering to one hMSC was counted manually for 100 MSCs for each technical
replicate. Fluorescent images were digitally re-stained (INA-6 green, hMSC
inverse black).


\subsubsection*{Analysis of INA-6 Survival and Aggregation Depending on hMSC Confluence}
To describe aggregate growth and survival of INA-6 depending on hMSC density,
...
constant ratios of INA-6 to adhesion surface provided by hMSCs. Those plates
...
focusing on each tile both before and after harvesting. Afterwards, INA-6 cells
...
\begin{table}[h]
    \footnotesize
    \centering
    \caption*{\textbf{Seeding Table:} Seeding densities for describing growth and survival of INA-6 depending on hMSC density. Co-cult. dur.: Co culturing duration; MSC-adh. surface: adhesion surface provided by hMSCs; vol.: volume.}
    \label{tab:co_culture_conditions}
    \begin{tabular}{|p{2cm}|p{2cm}|p{2cm}|p{3.5cm}|p{2cm}|p{3cm}|}
        \hline
        \textbf{Co-cult. dur. [h]} & \textbf{hMSC density [1000 hMSC/cm\textsuperscript{2}]} & \textbf{INA-6 density [1000 INA6/cm\textsuperscript{2}]} & \textbf{Ratios INA : MSC (adh. surface)} & \textbf{Seeding vol. [\textmu L]} & \textbf{End vol. [\textmu L]}                                        \\
        \hline
...
For luminescent assessment of cell survival, INA-6 cells were harvested by
...
again with \SI{150}{\micro\liter} MSC-medium. Harvested INA-6 cells were
...
re-distributed into 96-well plates (white, clear bottom) with \SI{1e5}{INA-6 cells} per well, and then subjected to viability and apoptosis assays.

To minimize the loss of sensitive apoptotic cells, another approach was used to
measure viability and apoptosis without harvesting INA-6 cells. hMSCs and INA-6
...
four technical replicates measured in relative luminescent units per seconds
    [RLU/s], \(L_{\text{Co Culture}}\), \(L_{\text{MSC}}\), \(L_{\text{INA6}}\) the
luminescence measured in the co-culture, hMSCs alone, and INA-6 alone,
respectively.
...
\subsubsection*{Time-Lapse Characterization of INA-6 Aggregation, Detachment and Division}
In order to record the aggregation and detachment of INA-6 in contact with
...
of \SI{3}{\hour} (\SI{80}{\percent} humidity and \SI{5}{\percent} CO2). INA-6
...
interacting INA-6 cells and went backward in time to see if they were two
daughter cells or if two independent INA-6 cells had collided. We determined the
...
interaction partner, and the number of detaching INA-6 cells.
...
movements or “rolling”). The mobility criteria were met if one INA-6 daughter
cell moved farther than half a cell radius within one frame (\SI{15}{\minute})
relative to the MSC-adherent INA-6 cell which was required to stand still
...
INA-6 cells underwent apoptosis shortly after division.


\subsubsection*{Cell Cycle Synchronization at M-Phase}
INA-6 cells were arrested at mitosis by double thymidine (\SI{2}{mM}) treatments
followed by \SI{5}{\hour} of nocodazole (\SI{500}{ng/mL}) incubation. In detail:
\SI{3e5}{cells/mL} INA-6 in \SI{4}{mL} were treated with \SI{2}{mM} thymidine
(Sigma Aldrich GmbH) for \SI{16.5}{\hour}. Cells were released by washing them
in INA-6 medium once and allowed to cycle for \SI{9}{\hour} before treating them
...
INA-6 were released by washing them once and resuspending them in MSC-medium
with \SI{33}{\percent} MSC-conditioned medium. Cell cycle profile was checked
using image cytometry (\refapdx{apdx:supplemental}{fig:S2}).
...
ensure that only INA-6 are pelleted in the v-well tip, hMSCs were removed from
the well-tip by touching the well-ground with a \SI{10}{\micro\liter} pipette
and roughly pipetting hMSCs away.

Arrested INA-6 (\SI{1e4}{cells/cm^2}) were released by washing them once in PBS
and resuspending them in \SI{33}{\percent} (v/v) MSC-conditioned medium before
adding them on top of confluent hMSCs. INA-6 adhered for 1, 2, 3, and
\SI{24}{\hour} before the complete co-culture was stained with \SI{0.5}{\micro
    M} Calcein-AM (\SI{10}{\minute} at \SI{37}{\degreeCelsius}). Non-adherent INA-6
...
hMSCs. Reference brightness was acquired from a cell culture with only INA-6,
...
\SI{-20}{\degreeCelsius}. Remaining non-MSC-adhering INA-6 cells were removed by
replacing culture medium with \SI{100}{\micro\liter} of medium. MSC-adherent
INA-6 were manually detached by rapid pipetting and equally pelleted, analyzed,
and isolated.

\subsubsection*{Cell Cycle Profiling}
INA-6 cells were fixed in \SI{70}{\percent} ice-cold ethanol, washed,
...
package LMFIT \cite{newvilleLMFITNonLinearLeastSquare2014} (\refapdx{apdx:supplemental}{fig:S4}).
...
For validation of image cytometry, \SI{5}{mL} of INA-6 stock culture was removed
...
\begin{itemize}
    \item \textbf{Collect MSC-Conditioned Medium and Add INA-6:}
          \begin{enumerate}
              ...
              \item Count INA-6 cells and retrieve enough cells to fill all 96 wells with \SI{2e4}{INA6/cm^2} (\SI{6.8e4}{cells} per well, covering approximately \SI{65}{\percent} of the well bottom).
              \item Centrifuge INA-6 (\SI{5}{minutes}, \SI{250}{g}) and resuspend them in a volume of diluted CM to reach a concentration of \SI{6.8e5}{INA6/mL}.
              \item Add \SI{100}{\micro\liter} INA-6 suspension to hMSCs (end volume: \SI{200}{\micro\liter}; end concentration: \SI{33}{\percent} (v/v) hMSC-conditioned medium).
              \item Incubate for \SI{24}{hours} at \SI{37}{\degreeCelsius} and \SI{5}{\percent} CO$_2$.
          \end{enumerate}

    \item \textbf{Prepare CM-INA6 Reference:}
          \begin{enumerate}
              \setcounter{enumi}{7}
              \item Add \SI{100}{\micro\liter} of fresh MSC-medium into each well of an empty 96-well plate (not coated).
              \item Add \SI{100}{\micro\liter} of INA-6 suspension (\SI{6.8e5}{INA-6/mL} in diluted CM).
              \item Incubate for \SI{24}{hours} at \SI{37}{\degreeCelsius} and \SI{5}{\percent} CO$_2$.
          \end{enumerate}

    \item \textbf{Collect CM-INA6 and nMA-INA6:}
          \begin{enumerate}
              \setcounter{enumi}{10}
              ...
              \item Prepare well-plate sandwiches:
                    \begin{itemize}
                        \item[a.] Turn an empty 96-well plate (“catching plate”) upside down and place one on top of the co-culture-plate, the CM-INA6 reference plate, and the counter-weight so that all well openings align.
              ...
              \item Turn the co-culture plate around and add \SI{30}{\micro\liter} of washing medium (MSC-medium 0\% FCS, \SI{3}{mM} EDTA) gently by touching the wall of each well and pressing the pipette slowly.
                    \begin{itemize}
                        ...
                        \item[b.] Many nMA-INA6 are removed by physical force applied by adding \SI{30}{\micro\liter} of medium and not just by centrifugation. Hence, it is critical to apply the same dispensing technique across all replicates. We recommend using a multipette (Eppendorf) that can apply \SI{30}{\micro\liter} with controllable pressure, since its push-button retains a long pushing path even for dispensing small volumes, unlike push-buttons from the usual \SI{100}{\micro\liter} pipettes that reduce the pushing-path for smaller volumes.
              ...
              \item Pool CM-INA6 from the catching plate that was fixed to the reference plate.
              \item Pool nMA-INA6 from the catching plate that was fixed to the co-culture plate.
              \item Collect remaining INA-6 by adding \SI{100}{\micro\liter} of PBS into each well of the catching plates, collect and pool with CM-INA6 or nMA-INA6.
              \item Strain CM-INA6 and nMA-INA6 using \SI{40}{\micro\meter} cell strainer.
              \item Isolate MA-INA6 by continuing with either accutase dissociation or rough pipetting.
          \end{enumerate}

    \item \textbf{Collect MA-INA6 by Accutase Dissociation Followed by MAC Sorting:}
          \begin{enumerate}
              \setcounter{enumi}{24}
              ...
              \item Evaluate presence of adherent INA-6 cells and the integrity of confluent hMSCs under the microscope.
              \item Repeat steps 27-30 until all INA-6 cells have dissociated or until confluent hMSCs start to tear.
              \item Strain cell suspension (\SI{30}{\micro\meter}). This yields MA-MSC.
              \item Pellet MA-INA6, nMA-INA6, and CM-INA6 (\SI{1200}{rpm}, \SI{10}{minutes}).
              \item Resuspend MA-INA6 in \SI{86}{\micro\liter} sorting buffer (PBS, \SI{2}{mM} EDTA, \SI{1}{percent} BSA).
              \item Resuspend CM-INA6 and nMA-INA6 in \SI{300}{\micro\liter} cold diluted accutase and incubate for \SI{3}{minutes} at \SI{37}{\degreeCelsius} to ensure equal treatment for all samples.
              \item Stop accutase by adding \SI{200}{\micro\liter} of FCS (\SI{100}{percent}).
              \item Pellet CM-INA6 and nMA-INA6 (\SI{1200}{rpm}, \SI{10}{minutes}) and resuspend in \SI{86}{\micro\liter} sorting buffer (PBS, \SI{2}{mM} EDTA, \SI{1}{percent} BSA).
              ...
              \item Improve purity of eluted MA-INA6 by straining eluate (\SI{30}{\micro\meter}) (wash strainer using \SI{1}{mL} of sorting buffer) and applying it onto an MS column a second time. Wash three times.
              \item Collect \SI{20}{\micro\liter} per eluate and apply it onto a \SI{96}{well} plate to evaluate purity.
                    \begin{itemize}
                        ...
                        \item[b.] Count the number of adherent cells (hMSCs) per INA-6 using phase contrast microscopy.
                        \item[c.] We reached a mean purity of \SI{3.2e-4} (±\SI{2.2e-4}) hMSCs per MA-INA6.
    ...
    \item \textbf{Collect MA-INA6 by Rough Pipetting (No MAC Sorting):}
          \begin{enumerate}
              \setcounter{enumi}{46}
              ...
              \item Repeat steps 46-50 until a sufficient amount of MA-INA6 is removed.
              \item Control purity of MA-INA-6 by placing out aliquot onto an empty \SI{96}{well} plate.
              \item Collect MA-INA6 from catching plate.
              \item Remove hMSCs by straining cell suspension (\SI{30}{\micro\meter}).
              \item Count cells using a Neubauer chamber.
              \item Pellet MA-INA6 (\SI{250}{g} for \SI{5}{minutes}).
...
strong enough to dissociate INA-6 from hMSCs, but could help with removing INA-6
from other INA-6. For generating samples for RNAseq, we added \SI{3}{mM} of EDTA
...
adhesion depends on hMSC donor or internal variance of INA-6. We recommend using
...
To track the cell count during WPSC, INA-6 cells were stained with \textit{CellTracker
...
\subsubsection*{Sub-Culturing After WPSC of MSC-Interacting INA-6 Subpopulations}
After CM-INA6, nMA-INA6, and MA-INA6 were isolated, they were counted with a
...
Total RNA was isolated from INA-6 cells using the \textit{NucleoSpin RNA II
...
\subsubsection*{RNAseq, Differential Expression and Functional Enrichment Analysis of INA-6 cells}
...
(group=F1, F2, F3, which are abbreviations for CM-INA6, nMA-INA6, MA-INA6,
respectively)
...
Fold change expression was normalized by the median of CM-INA6 (and not
...
order to display variation of CM-INA6 next to nMA-INA6 and MA-INA6.
...
same batch of hMSCs and/or INA-6 cells on the same day. For analyses of
...
replicate or recording (see \refapdx{apdx:supplemental}{tab:S1}). Bars and lines represent the
...
through varying hMSCs donors across experiments (see \refapdx{apdx:supplemental}{tab:S1}).
