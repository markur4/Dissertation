...
\unnsubsection{Time-Lapse Microscopy Added Intuition to Exploratory Cell
    Biology}%
\label{sec:potential_breakthroughs}%
...
\unnsubsection{Semi-Automated Analysis was Required for Establishing Methods}%
\label{sec:discussion_semi_automated_analysis}%
In this work, the development and publication of innovative \textit{in vitro}
methodologies (Well Plate Sandwich Centrifugation and V-Well adhesion Assay)
necessitated the adoption of semi-automated data analysis tools. These novel
methods introduced complexities that span multiple experimental parameters,
making the results multidimensional. This demanded precise, efficient and
standardized data handling capabilities which were facilitated by tools like
\texttt{plotastic}.
...
research \cite{westReinventingResearchAgile2018}. This adaptive approach was
...
\textbf{Quick Exploration and (Re-)Analysis using \texttt{plotastic} } The
development of new methodologies for studying myeloma dissemination necessitated
frequent adjustments in data presentation, making \texttt{plotastic} an
essential tool for rapid visualization and iterative analysis. The ability to
swiftly rearrange data visualizations \dashedsentence{adjusting factors
    represented on the x-axis or altering color codes} is crucial, especially when
exploring novel research methodologies. \texttt{plotastic} enhances
\texttt{seaborn}'s existing functionalities with features like the
\texttt{.switch()} method, simplifying the exploration of different plot
structures through switching factors through \texttt{seaborn}'s facetting parameters
(\texttt{hue}, \texttt{row}, \texttt{col}, etc.). Its \texttt{multiplots} feature is
particularly valuable, enabling the automation of multiple \texttt{seaborn}
functions to maintain consistent plotting formats throughout the publication, a
critical requirement for many journals.
...
% method. However, \texttt{plotastic} was mostly required for its
