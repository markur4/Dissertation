...
% \citet{kuricPlotasticBridgingPlotting2024}, which was published in the
% \textit{Journal of Open Source Software} under the \texttt{CC BY 4.0} license.

{\footnotesize \noindent The following chapter has been taken with slight adjustments from
\citet{kuricPlotasticBridgingPlotting2024}, published in \textit{Journal of Open
Source Software} under the \texttt{CC BY 4.0} license, and was expanded with
introduction and discussion sections.}

% Writing of this thesis involved the use of generative AI models (\textit{ChatGPT4},
% ChatGPT4o and \textit{GitHub Copilot}) to improve phrasing and assist with programming,
...
% version of this thesis at \linebreak\texttt{github.com/markur4/Dissertation}.}
...
dependent on multiple experimental parameters (factors), like \textit{`time'} or
\textit{`kinds of treatments'} \cite{reblTimedependentMetabolicActivity2010,
...
(\mypageref{sec:discussion_semi_automated_analysis}), since this chapter focuses
...
\texttt{pingouin}, \texttt{seaborn}, \texttt{matplotlib} and \allowbreak
...
structures~\cite{wickhamTidyData2014, reback2020pandas,
    mckinney-proc-scipy-2010}.



\includeimage[0.85][2mm 2mm 2mm 2mm]{
    FIGS/C2_fmri2.png
}\figcaption[fig:examplefmri]{
    Example figure of \texttt{plotastic} (version 0.1). Image style was set
    by \texttt{plt.style.use(`ggplot')}. \selfplagiarismtwo
}
...
\begin{table}[H]
    ...
    \caption{Results from \texttt{DA.check\_sphericity()}. \texttt{plotastic}
        ...
        subsequent sphericity tests for four datasets. \selfplagiarismtwo}
...
\cite{ReadDocs2024, brandlSphinx2021}. This would not only ensure
...
% are only limited by their methodological expertise to
to formulate a correct prompt \cite{qureshiAreChatGPTLarge2023}\footquote{You can
now recognize and learn the language of almost anything with structure, and you
can translate it to anything with structure — so text-protein, protein-text.
[...] Everybody is a programmer, and the programming language of the future is
called `human.'}{kelleherNVIDIACEOThis2024}. For instance, when a software is
...
has garnered 10 \emph{stars} \dashed{similar to a
`like' on social media platforms} on its GitHub page. This level of engagement,
...
\apdxref{subapdx:classdiagr}). But further efforts are required, e.g. only few
...
% \subsubsection*{Usage of generative models}
