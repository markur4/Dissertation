...
% - Ignoring chemotactic signals:
% Does \ac{MM} ignore CXCL12 signals?

% Furthermore, hypoxic conditions within the BME induce changes in myeloma cell
% adherence to both BM stromal cells and collagen, easing their release.

% Hypoxia-inducible factors such as HIF-2α disrupt the CXCR4/CXCL12 retention
% signal, reducing myeloma cell responsiveness to these stromal-derived cues and
% promoting their exit from the bone marrow
...
from ``virtually every post-natal organs and tissues'', and not just bone marrow
\cite{dasilvameirellesMesenchymalStemCells2006}. However, depending on which
tissue they originated from, \acp{MSC} can differ greatly in their transcription
profile and \textit{in vivo} differentiation potential
...
Today, \acp{MSC} are a popular subject in regenerative medicine, but are also
crucial to understand the \ac{BMME} in the context of pathologies like multiple
myeloma. 
...
% ======================================================================
% ======================================================================
\unnsubsection{Molecular Targets of MSC­--Myeloma Interactions}
\label{sec:intro_myeloma_hMSC}
As mentioned in previous sections, MSCs are key drivers of \ac{MM} progression
through mediating retention and survival of \ac{MM} cells in the bone marrow
through e.g. cell adhesion and chemoattractants
\cite{zeissigTumourDisseminationMultiple2020}. However, \acp{MSC} play a far
...
crucial to revisit how myeloma cells impact the \ac{BMME} through their
interactions with \acp{MSC}.

In healthy bone, there's an equilibrium between bone formation and degradation
to maintain bone turnover, repair and remodelling.
\cite{vaananenMechanismBoneTurnover1993}: Mesenchymal stromal cells (MSCs)
...
\ac{MM} cells establish this shift in bone turnover by direct stimulation of
osteoclast activity \dashedsentence{through secretion of MIP1$\alpha$
\cite{obaMIP1alphaUtilizesBoth2005}}, but also through impeding \acp{MSC}
functions of differentiating into osteoblasts: \ac{MM} cells inhibit osteogenic
Wnt signaling by producing of DKK1 \cite{qiangDkk1inducedInhibitionWnt2008,
zhouDickkopf1KeyRegulator2013}.

Instead, \acp{MSC} are driven towards the adipogenic lineage \ac{MM} by VCAM1
signaling, with MM cells stabilizing the adipogenic transcription factor
PPAR-$\gamma$ \cite{dotterweichContactMyelomaCells2016,
liuMyelomaCellsShift2020}.



Direct contact is establishd by e.g. VCAM1 on MSCs, which activates NF-$\kappa$B
in both myeloma and MSCs, with both cells reacting differently
\cite{cippitelliRoleNFkBSignaling2023, royNFkBActivatingPathways2018}: In
Myeloma cells, NF-$\kappa$B transduces survival signaling in myeloma cells
\cite{royNoncanonicalNFkBMutations2017}, whereas \acp{MSC} react with
stress-induced senescence along with secretion of soluble survival factors
\cite{chauhanMultipleMyelomaCell1996, fairfieldMultipleMyelomaCells2020}. This
also impedes MSC function to regulate osteoclast activity, causing MSCs to
produce osteoclast stimulating factor RANKL \cite{tsubakiHGFMetNFkB2020}.




Myeloma cells also inhibit osteogenic
differentiation of \acp{BMSC} on an epigenetic level through GFI1, a zinc finger
protein activated by TNF-$\alpha$ which recruits the chromatin modifier HDAC1 to
the promoter of osteogenic transcription factor RUNX2
...
previously treated mice with the same inhibitor XRK3F2, which resulted in
bone formation restricted to MM containing bones.


Overall, many advances have been made in understanding the mechanisms of Myeloma
cell reprograming the \ac{BMME} to support their growth and survival. They
target especially \acp{MSC}, 

using both direct cell-cell contact to 
 soluble factors
and extracellular vesicles to reprogram them. 
induce NF-$\kappa$B
on both \acp{MSC} and myeloma cells,
Promising therapeutic targets
 are discovered through experiments studying \ac{MSC} and myeloma
interactions, which potentially helps treating \ac{MM} in the future.


% Previously, the same group showed that osteogenic differentiation is suppressed


% also bridges NF-$\kappa$B and Wnt signaling.

% inhibiting the NF-$\kappa$B pathway in


% showed that \ac{MM} cells suppress osteogenic
% \ac{MM} cells suppress osteogenic

% differentiation on an epigenetic level as well, since NF-$\kappa$B signaling
% activates Gfi1 in \ac{MSCs}, which in turn recruits the chromatin
% modifier HDAC1 to the promoter of osteogenic transcription factor RUNX2 \cite{}

% \cite{adamikXRK3F2InhibitionP62ZZ2018}.



% and, intriguingly,
% inhibition of Gfi1 could be sufficient to rescue
% \cite{adamikXRK3F2InhibitionP62ZZ2018}
...
% physiology~\cite{alcorta-sevillanoDecipheringRelevanceBone2020}. 
