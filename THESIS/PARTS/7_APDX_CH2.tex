...
\includeimage[0.45]{
    ...
    page. \umlconvention
}\label{fig:S_classdiagr}

\vspace*{-1cm}
% ## Lower Part
\includeimagecontinued[0.88][][%
    The architecture of \texttt{plotastic} continues after the class
    \texttt{DataIntegrity} with classes for plotting (\texttt{SubPlot}) and
    statistical testing (\texttt{StatTest}) and ends with the class
    \texttt{DataAnalysis}, which serves as the main user interface.
    \umlconvention ]{
    APPENDIX_CHAPTER2/classdiagr_plot+stats.png
}\label{fig:S_classdiagr_cont}
